...
/// </summary>
/// <remarks>
///     Internally keeps these <see cref="RedisConnection" /> implementations
///     keyed by "name" in a <see cref="ConcurrentDictionary{TKey, TValue}"/>.
/// </remarks>
public interface IRedisConnectionProvider
{
    /// <summary>
    ///     Retrive the pooled Redis connection from the
    ...
    /// </param>
    /// <returns>An existing <see cref="IRedisConnection" /> if found.</returns>
    /// <exception cref="InvalidOperationException">
    ///     Thrown if we are unable to find a mathing Redis Connection with <paramref name="name" />.
    /// </exception>
    IRedisConnection GetRequiredConnection(string name);
}
